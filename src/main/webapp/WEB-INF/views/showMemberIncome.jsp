...
<body>
          ...
          <div class="col-md-12 bordered">
			  ...
			  <div id="page-select" class="row">
	               <ul class="pagination" class="col-md-8" >
	               		<li ><a href="showMemberIncome?current=1&memberID=${memberID}" >首页</a></li>
	               		<li ><a href="showMemberIncome?current=${page.current-1 > 0 ?page.current-1:1}&memberID=${memberID}">&laquo;</a></li>
						<c:if test="${page.total lt 9}">
							<c:forEach  begin="1" end="${page.total}" var="j">
								<c:if test="${page.current eq j}">
									<li><a href="showMemberIncome?current=${page.current}&memberID=${memberID}" class="now">${j}</a></li>
								</c:if>
								<c:if test="${page.current ne j}">
									<li><a href="showMemberIncome?current=${j}&memberID=${memberID}">${j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current gt 4)}">
							<c:forEach  begin="-4" end="4" var="j">
								<c:if test="${page.current eq (page.current+j)}">
									<li><a href="showMemberIncome?current=${page.current+j}&memberID=${memberID}" class="now">${page.current+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.current+j)}">
									<li><a href="showMemberIncome?current=${page.current+j}&memberID=${memberID}">${page.current+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current lt 4)}">
							<c:forEach  begin="-8" end="${page.total}" var="j">
								<c:if test="${page.current eq (page.total+j)}">
									<li><a href="showMemberIncome?current=${page.total+j}&memberID=${memberID}" class="now">${page.total+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.total+j)}">
									<li><a href="showMemberIncome?current=${page.total+j}&memberID=${memberID}">${page.total+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<li><a href="showMemberIncome?current=${page.current+1>page.total?page.total:page.current+1}&memberID=${memberID}">&raquo;</a></li>
						<li id="li"><a href="showMemberIncome?current=${page.total}&memberID=${memberID}">尾页</a></li>
	               </ul>
	               <form class="col-md-3">
	...
	$("#toPageIpt").change(function(){
		...
		if(pageNum >=1 && pageNum <=${page.total}){
			$('#jump').attr("href","showMemberIncome?current="+pageNum+"&memberID="+${memberID});
		}
	});
	</script>
</body>
