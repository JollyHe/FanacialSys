          ...
          <div class="col-md-12 bordered">
			  ...
			  <div id="page-select" class="row">
	               <ul class="pagination" class="col-md-8" >
	               		<li ><a href="incomeShowDetails?current=1" >首页</a></li>
	               		<li ><a href="incomeShowDetails?current=${page.current-1 > 0 ?page.current-1:1}&id=${id}">&laquo;</a></li>
						<c:if test="${page.total lt 9}">
							<c:forEach  begin="1" end="${page.total}" var="j">
								<c:if test="${page.current eq j}">
									<li><a href="incomeShowDetails?current=${page.current}&id=${id}" class="now">${j}</a></li>
								</c:if>
								<c:if test="${page.current ne j}">
									<li><a href="incomeShowDetails?current=${j}&id=${id}">${j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current gt 4)}">
							<c:forEach  begin="-4" end="4" var="j">
								<c:if test="${page.current eq (page.current+j)}">
									<li><a href="incomeShowDetails?current=${page.current+j}&id=${id}" class="now">${page.current+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.current+j)}">
									<li><a href="incomeShowDetails?current=${page.current+j}&id=${id}">${page.current+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current lt 4)}">
							<c:forEach  begin="-8" end="${page.total}" var="j">
								<c:if test="${page.current eq (page.total+j)}">
									<li><a href="incomeShowDetails?current=${page.total+j&id=${id}}" class="now">${page.total+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.total+j)}">
									<li><a href="incomeShowDetails?current=${page.total+j}&id=${id}">${page.total+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<li><a href="incomeShowDetails?current=${page.current+1>page.total?page.total:page.current+1}&id=${id}">&raquo;</a></li>
						<li id="li"><a href="incomeShowDetails?current=${page.total}&id=${id}">尾页</a></li>
	               </ul>
	               <form class="col-md-3">
	...
	<script type="text/javascript">
		$("#toPageIpt").change(function(){
			...
			if(pageNum >=1 && pageNum <=${page.total}){
				$('#jump').attr("href","incomeShowDetails?current="+pageNum+"&id=${id}");
			}
		});
	</script>
</body>
