  ...
  <div class="cont_centrar">
        ...
        <div class="cont_form_login"> <a href="#" onClick="ocultar_login_sign_up()" ><i class="fa fa-arrow-right"></i></a>
          <h2>附加信息</h2>
          		<div class="selectGroup">
		          	<span>类型</span>
		          	<select name="log.logID" form="income" placeholder="便签" size="1"> 
						 <c:forEach items="${logs}" var="log" varStatus="status">
							<c:choose >
								<c:when test="${log.logID eq income.log.logID}">
          		...
          		<div class="selectGroup">
          			<span>收入组</span>
		         	 <select name="incomegroup.incomegroupID" form="income" placeholder="便签" size="1">      
							  <c:forEach items="${groups}" var="group" varStatus="status">
							  <c:choose >
								  <c:when test="${group.incomegroupID eq income.incomegroup.incomegroupID }">
                ...
                <div class="selectGroup">
          			<span>计划</span>
		         	 <select name="plan.planID" form="income" placeholder="便签" size="1"> 
         ...
         <form id="income" action="/FinacialSys/editIncome/${current}/${id}" method="post">
	        <div class="cont_form_sign_up"> <a href="#" onClick="ocultar_login_sign_up()"><i class="fa fa-arrow-left"></i></a>
	          <h2>基本信息</h2> 
	          	
	          				  <input type="text" value="${ income.inMoney}" name="inMoney"/>
					          <input type="text" value="${ income.inPlace }" name="inPlace"/>
					          <input type="text" value="${ income.inMessage }" name="inMessage"/>
					          <input type="text" value="${ income.inTime }" name="inTime" id="incomeDate"/>
	          		
		         
		          <button class="btn_sign_up" type="submit" form="income">提交</button>      
	        </div>
         </form>
      </div>
    </div>
	...
	<script type="text/javascript">
		!function(){
	  		...
	  		laydate({elem: '#incomeDate', max: laydate.now(0)});//绑定元素
	  	}();
	</script>
</body>
</html>