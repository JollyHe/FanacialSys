...
<body>
          ...
          <div class="col-md-12 bordered">
					...
					<c:if test="${page.list.size() lt 10}">
						<c:forEach items="${page.list}" var="incomeGroup">
							<tr>
								...
								<td>
									<a href="showIncomeGroupDetails/${page.current}/${incomeGroup.incomegroupID}">查看详情</a>
									<c:if test="${ !incomeGroup.incomegroupBrief.equals('其他') }">
										<a href="editIncomeGroup/${page.current}/${incomeGroup.incomegroupID}">修改</a>
										<a href="incomeGroupShow/removeIncomeGroup/${page.current}/${incomeGroup.incomegroupID}">删除</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
			  ...
			  <div id="page-select" class="row">
	               <ul class="pagination" class="col-md-8" >
	               		<li ><a href="incomeGroupShow?current=1" >首页</a></li>
	               		<li ><a href="incomeGroupShow?current=${page.current-1 > 0 ?page.current-1:1}">&laquo;</a></li>
						<c:if test="${page.total lt 9}">
							<c:forEach  begin="1" end="${page.total}" var="j">
								<c:if test="${page.current eq j}">
									<li><a href="incomeGroupShow?current=${page.current}" class="now">${j}</a></li>
								</c:if>
								<c:if test="${page.current ne j}">
									<li><a href="incomeGroupShow?current=${j}">${j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current gt 4)}">
							<c:forEach  begin="-4" end="4" var="j">
								<c:if test="${page.current eq (page.current+j)}">
									<li><a href="incomeGroupShow?current=${page.current+j}" class="now">${page.current+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.current+j)}">
									<li><a href="incomeGroupShow?current=${page.current+j}">${page.current+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<c:if test="${(page.total eq 9) && (page.total-page.current lt 4)}">
							<c:forEach  begin="-8" end="${page.total}" var="j">
								<c:if test="${page.current eq (page.total+j)}">
									<li><a href="incomeGroupShow?current=${page.total+j}" class="now">${page.total+j}</a></li>
								</c:if>
								<c:if test="${page.current ne (page.total+j)}">
									<li><a href="incomeGroupShow?current=${page.total+j}">${page.total+j}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						<li><a href="incomeGroupShow?current=${page.current+1>page.total?page.total:page.current+1}">&raquo;</a></li>
						<li id="li"><a href="incomeGroupShow?current=${page.total}">尾页</a></li>
	               </ul>
	               <form  class="col-md-3">
	...
	$("#toPageIpt").change(function(){
		...
		if(pageNum >=1 && pageNum <=${page.total}){
			$('#jump').attr("href","incomeGroupShow?current="+pageNum);
		}
	});
	</script>
</body>
